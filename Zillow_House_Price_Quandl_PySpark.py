...
zillow_regions.filter('region_id IS NULL').count()  # check number of null values 0
zillow_data.createOrReplaceTempView("zillow_data_temp") #create a Table/View from the PySpark DataFrame to use sql queries on
...
zillow_data.select('indicator_id').distinct().show() #show distinct indicator values
...
metro_URL = 'https://en.wikipedia.org/wiki/Metropolitan_statistical_area'  # this wiki page has table of metro areas
res = requests.get(metro_URL)  # get the request object of this URL
try: #check if the URL request was successful
    res.raise_for_status()
except Exception as exc:
    print('there was a problem: %s' % exc)
soup = BeautifulSoup(res.text)  # create beautiful soup object of request html text string
...
for row in range(len(soup.find('table', class_='wikitable').find_all('tr')[1:])):  # rang(0,384)
    ...
    try:
        ...
        relevant_html_text = html_data[
            0].getText()  # get the key text of the html, which includes the metro area in the 1st name of the string...runs into list index out of range error, can't proceed
        try:
...
metro_dict = {'metros': metro_list}  #create 1 key dictionary to list all the metros
...
metro_list_df_backup = pd.read_csv('metro_list_df_backup1.csv') #read back in the metro names
...
zillow_regions_new = spark.sql("SELECT * FROM zillow_regions_temp WHERE region_type = 'zip'")
...
spark.conf.set("spark.sql.execution.arrow.pyspark.enabled", "true") #optimizes conversion between spark df and pandas df via arrow
spark.conf.set("spark.sql.execution.arrow.pyspark.fallback.enabled", "true") #Ensures arrow is used even if errors occur
...
zillow_regions_new_pandas.loc[:, 'zip_code'] = zillow_regions_new_pandas.apply(lambda x: re.search('(\d{5})', x['region']).group(),
                                                                 axis=1)  # find the 5 digit element in the region column, store it in a new column zip_code

zillow_regions_new_pandas.loc[:, 'state'] = zillow_regions_new_pandas.apply(lambda x: check_state_in_str(x['region']),
                                                              axis=1)  # find state in the region column, store it in state column

zillow_regions_new_pandas.loc[:, 'county'] = zillow_regions_new_pandas.apply(lambda x: check_county_in_str(x['region']),
                                                               axis=1)  # find county in the region column, store it in county column

#get city down below using zip --> city calculation?
zillow_regions_new_pandas.loc[:, 'city'] = zillow_regions_new_pandas.apply(lambda x: check_city_in_str(x['region']),
                                                             axis=1)  # find city in the region column, store it in city column

zillow_regions_new_pandas.loc[:, 'metro'] = zillow_regions_new_pandas.apply(lambda x: check_metro_in_str(x['region']),
...
zillow_regions_new_pandas_to_pyspark.createOrReplaceTempView('zillow_regions_new_pandas_to_pyspark_temp')
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state IS NULL").show() #2166 states are null
zillow_regions_new_pandas_to_pyspark.filter('city IS NULL').count() #3867 rows where city is null

#read in the zip to state file
zip_to_state = spark.read.csv('uszips.csv',
                              header = True,
                              inferSchema=True).withColumnRenamed('city', 'city2')
...
zip_to_metro = spark.read.csv('zip_to_metro.csv',
                              header = True,
                              inferSchema=True).withColumnRenamed('ZIP', 'ZIP_metro').withColumnRenamed('Metro', 'Metro_metro')

#replace "[not in a CBSA]" value with null
from pyspark.sql.functions import col,when
zip_to_metro = zip_to_metro.withColumn('Metro_metro', \
                                       when(col("Metro_metro") == '[not in a CBSA]', None) \
                                       .otherwise(col("Metro_metro")))
...
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.\
    join(zip_to_state, zillow_regions_new_pandas_to_pyspark.zip_code == zip_to_state.zip, 'left')
...
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.\
    join(zip_to_metro, zillow_regions_new_pandas_to_pyspark.zip_code == zip_to_metro.ZIP_metro, 'left')

#count how many times state and state_id are null, different, prechecks
zillow_regions_new_pandas_to_pyspark.createOrReplaceTempView('zillow_regions_new_pandas_to_pyspark_temp')
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state == state_id").show() #35451 rows
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state <> state_id").show() #50 rows
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state IS NULL").show() #2166 rows
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state_id IS NULL").show() #425 rows
spark.sql("SELECT zip_code, zip, state, state_id FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state_id IS NULL").show() #425 rows
...
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.withColumn("state", coalesce(zillow_regions_new_pandas_to_pyspark.state, zillow_regions_new_pandas_to_pyspark.state_id))
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.withColumn("city", coalesce(zillow_regions_new_pandas_to_pyspark.city, zillow_regions_new_pandas_to_pyspark.city2))
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.withColumn("county", coalesce(zillow_regions_new_pandas_to_pyspark.county, zillow_regions_new_pandas_to_pyspark.county_name))
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.withColumn("metro", coalesce(zillow_regions_new_pandas_to_pyspark.metro, zillow_regions_new_pandas_to_pyspark.Metro_metro))

#count how many times state and state_id are null, different, postchecks. Now state column is 100% populated.
zillow_regions_new_pandas_to_pyspark.createOrReplaceTempView('zillow_regions_new_pandas_to_pyspark_temp')
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state == state_id").show() #30712 rows
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state <> state_id").show() #52 rows
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state IS NULL").show() #0 rows
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state_id IS NULL").show() #425 rows
...
zillow_regions_new_pandas_to_pyspark_condensed = zillow_regions_new_pandas_to_pyspark.\
    drop('region_type', 'region', 'region_str_len', 'zip', 'lat', 'lng', 'city2', 'state_id', 'state_name',
         'zcta', 'parent_zcta', 'county_fips', 'county_name', 'county_weights', 'county_names_all', 'county_fips_all', 'imprecise',
         'military', 'timezone', 'ZIP_metro', 'Metro_metro')
...
zillow_temp = zillow_data.join(zillow_regions_new_pandas_to_pyspark_condensed, ['region_id'])
#zillow_temp.show(1)
...
try:
    ...
    print("Likely Error - Python was not found; run without arguments to install from the Microsoft Store, or disable this shortcut from Settings > Manage App Execution Aliases. Or java.io.IOException: Cannot run program python3: CreateProcess error=3, The system cannot find the path specified")
...
zillow_both = zillow_all.sample(fraction=.01)  # due to size of the data, just keep .1%, otherwise writing to csv was failing

zillow_all_house.count() #57611750
zillow_house = zillow_all_house.sample(fraction=.01)  # due to size of the data, just keep .1%, otherwise writing to csv was failing
...
zillow_all_rental.select([count(when(isnull(c), c)).alias(c) for c in zillow_all_rental.columns]).show() #count null values for all columns
...
zillow_all_drop_nulls = zillow_all_rental.na.drop(subset=['state', 'density', 'value']).drop('metro')
zillow_all_drop_nulls.show(5)

#recount after dropping nulls
zillow_all_drop_nulls.select([count(when(isnull(c), c)).alias(c) for c in zillow_all_drop_nulls.columns]).show() #count null values for all columns
zillow_all_drop_nulls.show(5)
...
encoder = OneHotEncoder(inputCols=['stateIndex'], outputCols=['stateOHE']) ## Create an instance of the one hot encoder

print("Vectorizing...")
assembler = VectorAssembler(inputCols=['stateOHE', 'density'], outputCol='features') # Create an assembler object, create vector of features, for now follow Datacamp idea and use these 2 columns
...
predictionModel_regression_train.select('stateOHE', 'features', 'value', 'prediction').show(5)
...
tvs = TrainValidationSplit(estimator=pipeline_regression,
...
params = params.addGrid(regression_model.regParam, [0.01, 0.1, 1.0, 10.0]) \
               .addGrid(regression_model.elasticNetParam, [0.0, 0.5, 1.0])
...
cv_model = CrossValidator(estimator=pipeline_regression,
...
indexer = StringIndexer(inputCol="state", outputCol="stateIndex")
zillow_indexed = indexer.fit(zillow_all_drop_nulls).transform(zillow_all_drop_nulls) #identifies categories in the data, then create a new column with numeric index values
#zillow_indexed.show()

#One-hot encoding
from pyspark.ml.feature import OneHotEncoder
encoder = OneHotEncoder(inputCols=['stateIndex'], outputCols=['stateDummy']) ## Create an instance of the one hot encoder
model = encoder.fit(zillow_indexed) # Apply the one hot encoder to the zillow data
zillow_encoded = model.transform(zillow_indexed)  # Apply the one hot encoder to the zillow data
...
try:
    ...
    zillow_encoded = zillow_encoded.withColumn("density", zillow_encoded.density.cast('int'))
    zillow_encoded.printSchema() #density is currently double, have to make it int

    from pyspark.ml.feature import RobustScaler
    scaler = RobustScaler(inputCol="density", outputCol="scaledDensity",
...
assembler = VectorAssembler(inputCols=['stateIndex', 'density'], outputCol='features') # Create an assembler object, create vector of features, for now follow Datacamp idea and use these 2 columns
...
zillow_condensed = zillow_assembled.select('state', 'stateIndex', 'density', 'value', 'features')
...
regression_model = LinearRegression(labelCol='value', regParam=.3).fit(zillow_train)
...
elnet_regression = LinearRegression(labelCol='value', regParam=.5, elasticNetParam=.5).fit(zillow_train)

# Calculate the RMSE on testing data
elnet_rmse = RegressionEvaluator(labelCol='value').evaluate(elnet_regression.transform(zillow_test))
...
encoder = OneHotEncoder(inputCols=['stateIndex'], outputCols=['stateDummy']) ## Create an instance of the one hot encoder
assembler = VectorAssembler(inputCols=['stateIndex', 'density'], outputCol='features') # Create an assembler object, create vector of features, for now follow Datacamp idea and use these 2 columns
regression_model = LinearRegression(labelCol='value', regParam=.3)
elnet_regression = LinearRegression(labelCol='value', regParam=.5, elasticNetParam=.5)
rf_model = RandomForestRegressor(labelCol='value', maxBins=52)
pipeline = Pipeline(stages=[indexer, encoder, assembler, regression_model, elnet_regression, rf_model])
...
pipeline = pipeline.fit(zillow_train) #error, saying state does not exist...add it back in when zillow_train is created
...
zillow_all_drop_nulls_date = zillow_all_drop_nulls.\
    select("*", to_date(col('date'), 'YYYY-MM-DD').alias('DateOfSale')) #.toPandas().astype  #plot(x='date', y = 'value')
...
zillow_all_rental.value = pd.to_numeric(zillow_all_rental['value']) #convert value to numeric
zillow_all_rental.date = pd.to_datetime(zillow_all_rental['date'], format='%Y-%m-%d') #convert date to date
zillow_all_rental.region_id = zillow_all_rental['region_id'].astype(str)
zillow_price_by_date = zillow_all_rental.groupby(['date'], as_index=False).median(['value']) #this is our time series data
...
print('p-value: ', adfuller_test[1]) #p-value = .99, cannot reject Ho that time series is non-stationary
...
axes[0, 0].plot(zillow_price_by_date.value); axes[0, 0].set_title('Original Series')
plot_acf(zillow_price_by_date.value, ax=axes[0, 1])

# 1st Differencing
axes[1, 0].plot(zillow_price_by_date.value.diff()); axes[1, 0].set_title('1st Order Differencing')
plot_acf(zillow_price_by_date.value.diff().dropna(), ax=axes[1, 1])

# 2nd Differencing
axes[2, 0].plot(zillow_price_by_date.value.diff().diff()); axes[2, 0].set_title('2nd Order Differencing')
...
axes[0].plot(zillow_price_by_date.value.diff()); axes[0].set_title('1st Differencing')
axes[1].set(ylim=(0,5))
...
fc = ARIMA_fitted_train.forecast(steps=len(test.index))  # create forecast for len(test.index) items
...
# Forecast
n_periods = 24
fc, confint = auto_arima_model.predict(n_periods=n_periods, return_conf_int=True) #make predictions for the next 24 periods
index_of_fc = np.arange(len(zillow_price_by_date.value), len(zillow_price_by_date.value)+n_periods) #store index values of forecasted data

# make series for plotting purpose
fc_series = pd.Series(fc, index=index_of_fc) #turn fc array into fc_series pandas series
...
zillow_price_by_date = zillow_price_by_date.set_index('date') #make the date an index for sarima to work
...
axes[1].plot(zillow_price_by_date[:].diff(12), label='Seasonal Differencing', color='green')
...
index_of_fc = pd.date_range(zillow_price_by_date.index[-1], periods = n_periods, freq='MS')
...
train = zillow_price_by_date[zillow_price_by_date.date < pd.to_datetime('2020-01-01', format='%Y-%m-%d')]
test = zillow_price_by_date[zillow_price_by_date.date >= pd.to_datetime('2020-01-01', format='%Y-%m-%d')]
...
y_pred = ARMAmodel.get_forecast(len(test.index)) #predict 29 out-of-sample forecasts
y_pred_df = y_pred.conf_int(alpha = 0.05) #generate a 95% confidence interval on the above predictions
y_pred_df["Predictions"] = ARMAmodel.predict(start = y_pred_df.index[0], end = y_pred_df.index[-1]) #use ARMA model to predict the values at test index 288-316
...
#generate predictions on test data using model
y_pred = ARIMAmodel.get_forecast(len(test.index)) #predict 29 out-of-sample forecasts
y_pred_df = y_pred.conf_int(alpha = 0.05) #generate a 95% confidence interval on the above predictions
y_pred_df["Predictions"] = ARIMAmodel.predict(start = y_pred_df.index[0], end = y_pred_df.index[-1]) #use ARMA model to predict the values at test index 288-316
y_pred_df.index = test.index #set test index to y pred index...
y_pred_out = y_pred_df["Predictions"] #store the predictions
#plot results
...
#generate predictions on test data using model
y_pred = SARIMAXmodel.get_forecast(len(test.index)) #predict 29 out-of-sample forecasts
y_pred_df = y_pred.conf_int(alpha = 0.05) #generate a 95% confidence interval on the above predictions
y_pred_df["Predictions"] = SARIMAXmodel.predict(start = y_pred_df.index[0], end = y_pred_df.index[-1]) #use ARMA model to predict the values at test index 288-316
y_pred_df.index = test.index #set test index to y pred index...
y_pred_out = y_pred_df["Predictions"] #store the predictions
#plot results
...
#calculate RMSE
sarimax_rmse = np.sqrt(mean_squared_error(test["value"].values, y_pred_df["Predictions"]))
