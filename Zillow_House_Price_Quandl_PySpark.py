...
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state IS NULL").show()
...
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.\
    join(zip_to_state, zillow_regions_new_pandas_to_pyspark.zip_code ==
         zip_to_state.zip, 'left')
...
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.\
    join(zip_to_metro, zillow_regions_new_pandas_to_pyspark.zip_code ==
         zip_to_metro.ZIP_metro, 'left')

# count how many times state and state_id are null, different, prechecks
zillow_regions_new_pandas_to_pyspark.createOrReplaceTempView(
    'zillow_regions_new_pandas_to_pyspark_temp')
# 35451 rows
spark.sql("SELECT count(*) FROM zillow_regions_new_pandas_to_pyspark_temp WHERE state == state_id").show()
...
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.withColumn("state", coalesce(
    zillow_regions_new_pandas_to_pyspark.state, zillow_regions_new_pandas_to_pyspark.state_id))
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.withColumn(
    "city", coalesce(zillow_regions_new_pandas_to_pyspark.city, zillow_regions_new_pandas_to_pyspark.city2))
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.withColumn("county", coalesce(
    zillow_regions_new_pandas_to_pyspark.county, zillow_regions_new_pandas_to_pyspark.county_name))
zillow_regions_new_pandas_to_pyspark = zillow_regions_new_pandas_to_pyspark.withColumn("metro", coalesce(
    zillow_regions_new_pandas_to_pyspark.metro, zillow_regions_new_pandas_to_pyspark.Metro_metro))
...
zillow_both = zillow_all.sample(fraction=.01)

zillow_all_house.count()  # 57611750
# due to size of the data, just keep .1%, otherwise writing to csv was failing
zillow_house = zillow_all_house.sample(fraction=.01)
...
zillow_all_rental.select([count(when(isnull(c), c)).alias(
    c) for c in zillow_all_rental.columns]).show()  # count null values for all columns
...
zillow_all_drop_nulls.select([count(when(isnull(c), c)).alias(
    c) for c in zillow_all_drop_nulls.columns]).show()  # count null values for all columns
zillow_all_drop_nulls.show(5)

# Split into training and testing sets in a 80:20 ratio
zillow_train, zillow_test = zillow_all_drop_nulls.randomSplit(
    [0.8, 0.2], seed=17)
...
categoricalColumns = ["indicator_id", "zip_code",
                      "state", "county", "city", "category"]
...
assembler = VectorAssembler(
    inputCols=['stateOHE', 'density'], outputCol='features')
...
predictionModel_regression_train.select(
    'stateOHE', 'features', 'value', 'prediction').show(5)
...
predictionModel_regression_test.select(
    'stateOHE', 'features', 'prediction').show(5)
...
zillow_indexed = indexer.fit(
    zillow_all_drop_nulls).transform(zillow_all_drop_nulls)
# zillow_indexed.show()
...
# Create an assembler object, create vector of features, for now follow Datacamp idea and use these 2 columns
assembler = VectorAssembler(
    inputCols=['stateIndex', 'density'], outputCol='features')
...
regression_model = LinearRegression(
    labelCol='value', regParam=.3).fit(zillow_train)
...
elnet_regression = LinearRegression(
    labelCol='value', regParam=.5, elasticNetParam=.5).fit(zillow_train)
...
rf_model = RandomForestRegressor(
    labelCol='value', maxBins=52).fit(zillow_train)
...
elnet_regression = LinearRegression(
    labelCol='value', regParam=.5, elasticNetParam=.5)
rf_model = RandomForestRegressor(labelCol='value', maxBins=52)
pipeline = Pipeline(stages=[indexer, encoder, assembler,
                    regression_model, elnet_regression, rf_model])
...
zillow_all_drop_nulls_date = zillow_all_drop_nulls.\
    select("*", to_date(col('date'), 'YYYY-MM-DD').alias('DateOfSale')
...
def time_split_train_test(df, time_series_splits, seasonality=seasonality):
    ...
    for i, (train_i, test_i) in enumerate(time_series_splits.split(df)):
        ...
        df_sub["rmse"] = (np.mean((df_sub.yhat - df_sub.y) ** 2)) ** .5
