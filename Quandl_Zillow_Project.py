...
zillow_data = zillow_data.sample(frac=.001)  # due to size of the data, just keep .1% for now
...
zillow_indicators['indicator'].nunique()  # check unique values of indicators (56)
...
soup = BeautifulSoup(res.text, 'lxml')  # create beautiful soup object of request html text string
...
zillow_all = zillow_data.merge(zillow_regions_new, on='region_id').merge(zillow_indicators, on='indicator_id')
...
zillow_all.date = pd.to_datetime(zillow_all['date'], format='%Y-%m-%d') #convert date to date
zillow_all.region_id = zillow_all['region_id'].astype(str)
zillow_price_by_date = zillow_all.groupby(['date'], as_index=False).median(['value']) #this is our time series data
