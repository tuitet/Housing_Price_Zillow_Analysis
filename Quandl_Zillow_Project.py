...
zillow_regions_new.loc[:, "region_str_len"] = zillow_regions_new.apply(
    lambda x: len(x["region"].split(";")), axis=1
)  # split the region column by ;, get the length of that object, store it in a new column called region_str_len
zillow_regions_new.loc[:, "zip_code"] = zillow_regions_new.apply(
